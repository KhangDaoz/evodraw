...
> **Mục đích của file này.** Đây là phiên bản viết lại của Chương 2 và Chương 3 trong báo cáo Thực tập cơ sở "Xây dựng nền tảng hỗ trợ làm việc nhóm từ xa tương tác trực quan với bảng trắng EvoDraw". Phiên bản này thay thế phương pháp luận **Phân tích và Thiết kế hướng đối tượng (OOAD)** trong báo cáo gốc bằng **Phân tích và Thiết kế hướng sự kiện (Event-Driven Analysis & Design — EDAD)**, và đặt tên rõ ràng kiến trúc phần mềm là **Event-Driven Architecture với Broker Topology, mở rộng từ mô hình MVC**.
>
...
**Yêu cầu thực hiện:** Vẽ lại các Hình 1.2, 1.3, 1.4 bằng Lucidchart/Draw.io theo đúng chiều bảng trên rồi thay thế trong báo cáo cuối cùng.
...
Đề tài áp dụng **Phương pháp Phân tích và Thiết kế hướng sự kiện (Event-Driven Analysis & Design — EDAD)**, sử dụng **Ngôn ngữ Mô hình hóa Thống nhất (UML)** làm công cụ biểu diễn chính.

**Vì sao chọn EDAD thay vì OOAD truyền thống?** Bản chất nghiệp vụ của EvoDraw là một hệ thống mà **mọi tương tác có ý nghĩa đều là một sự kiện** được phát tới nhiều bên quan tâm: một nét vẽ là sự kiện `canvas_op` tới mọi thành viên; một tin nhắn là sự kiện `chat:message`; một thao tác bắt đầu chia sẻ màn hình là sự kiện `screen:start`. OOAD cổ điển — vốn mô hình hóa hệ thống bằng các phương thức được gọi tuần tự giữa các đối tượng — không phản ánh được tính chất bất đồng bộ, một-nhiều, và độc lập về thời gian của những tương tác này. EDAD đặt **sự kiện (Event)** làm đơn vị mô hình hóa trung tâm, mô tả hệ thống dưới dạng những thành phần phát sinh sự kiện (Producer), những kênh chuyển tải sự kiện (Channel/Broker), và những thành phần phản ứng với sự kiện (Consumer/Event Handler).

EDAD trong báo cáo này **không vứt bỏ UML** mà tái diễn giải các biểu đồ UML theo tư duy hướng sự kiện:

| Biểu đồ UML | Vai trò trong EDAD |
| :--- | :--- |
| **Use Case Diagram** | Xác định actor và các use case **khởi phát Domain Event** (event-triggering use cases). |
| **Class Diagram** | Thể hiện 3 stereotype: `«View»` (thành phần giao diện phát UI event), `«Controller/EventHandler»` (đăng ký lắng nghe và phản ứng với event qua kênh broker, ghi rõ `_subscribesTo` và `_emits`), `«Entity»` (Aggregate/Mô hình dữ liệu được mutate khi event xảy ra). |
| **Sequence Diagram** | Mô tả luồng event theo trình tự thời gian. **Lifeline `«Broker»` được vẽ tường minh** giữa Producer và Consumer; mọi giao tiếp xuyên máy được biểu diễn là `emit(event_name)` đi vào Broker và `on(event_name)` đi ra Broker — không vẽ tắt từ máy A → máy B. |
| **State Diagram** (bổ sung) | Mô hình hóa vòng đời của các Aggregate có chuyển trạng thái rõ rệt (Room, ScreenShareSession, OverlayMode). |

Bốn khái niệm nền tảng của EDAD được sử dụng xuyên suốt báo cáo:

1. **Domain Event** — sự kiện nghiệp vụ đã xảy ra trong quá khứ (vd. `canvas_op`, `chat:message`, `screen:start`, `user_joined`).
2. **Event Channel** — kênh logic nhóm các Domain Event có cùng chủ đề (vd. kênh `canvas.sync`, kênh `chat`, kênh `screen`).
...
- `«Entity»` — lớp thực thể giữ trạng thái và **chứa phương thức nghiệp vụ** tương ứng. Việc gán phương thức vào Entity (thay vì tách ra Controller) là quy ước của giáo trình ở pha phân tích.
...
#### b) Class diagram phân tích cho từng module

Mỗi module được trình bày bằng một class diagram phân tích gồm hai stereotype `«View»` và `«Entity»`. **Phương thức nghiệp vụ được gán vào Entity tương ứng** — theo đúng phong cách giáo trình 7.8.3 ("*cần chức năng `<tên hàm>()` → chức năng này là hành động của đối tượng `<tên thực thể>`*"). Bảng đặc tả dưới đây thay cho hình vẽ; ở báo cáo cuối sẽ chuyển sang biểu đồ UML bằng Lucidchart.

---

##### *Module a — Chức năng Tạo phòng mới*

**Phân tích tĩnh.**

* NSD mở ứng dụng → giao diện trang chủ hiện ra → đề xuất lớp **LandingPage** `«View»`, có nút "Tạo phòng" và form "Tham gia phòng".
* NSD click "Tạo phòng" → hệ thống cần sinh mã phòng + PIN và lưu phòng → cần chức năng `create()` → đây là hành động của đối tượng **Room** `«Entity»`.
* Tạo xong, **LandingPage** hiển thị cặp mã và chuyển NSD vào giao diện phòng.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **LandingPage** | Hiển thị bảng trắng cục bộ + bảng chào (form "Tham gia phòng"). Bắt thao tác tương tác đầu tiên với bảng (chọn công cụ / click canvas) để yêu cầu tạo phòng mới. |
| `«Entity»` | **Room** | `create()` — sinh mã phòng và mã PIN ngẫu nhiên, lưu phòng và trả về cặp mã. |
...
classDiagram
    ...
    LandingPage --> Room : yeu cau tao
```

> *Hình 3.2: Class diagram phân tích — Tạo phòng mới.*

---

##### *Module b — Chức năng Tham gia phòng*

**Phân tích tĩnh.**

* NSD nhập mã phòng + PIN ở trang chủ → sử dụng lại lớp **LandingPage** `«View»`; bổ sung **JoinPage** `«View»` cho luồng vào bằng link mời.
* NSD click "Tham gia" → hệ thống cần kiểm tra mã phòng + PIN → cần `verifyPasscode(code, pin)`, `findByCode(code)` → là hành động của **Room** `«Entity»`.
* Hợp lệ → hệ thống mở giao diện phòng → đề xuất lớp **RoomPage** `«View»`.
* Hệ thống cần thêm NSD vào danh sách và liệt kê thành viên → cần `join(room)`, `listAll(room)` → là hành động của **Member** `«Entity»`.
* Hệ thống cần tải nội dung bảng trắng hiện có → cần `loadSnapshot()` (Room) và `synchronize()` để dựng lại các phần tử → liên quan **CanvasElement** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **LandingPage** / **JoinPage** | Form nhập mã phòng (6 ký tự) và mã PIN (4 chữ số) + nút "Tham gia"; hoặc tự động tham gia qua link mời. |
| `«View»` | **RoomPage** | Khung phòng làm việc, hiển thị bảng trắng và danh sách thành viên. |
| `«Entity»` | **Room** | `verifyPasscode(code, pin)` — kiểm tra mã phòng và PIN; `findByCode(code)` — tra cứu phòng; `loadSnapshot()` — tải nội dung bảng trắng hiện có. |
| `«Entity»` | **Member** | `join(room)` — thêm thành viên vào phòng; `listAll(room)` — liệt kê thành viên trong phòng. |
| `«Entity»` | **CanvasElement** | `synchronize()` — đồng bộ tới các thành viên cùng phòng. |
...
classDiagram
    ...
    LandingPage --> Room : xac thuc
    JoinPage --> Room : xac thuc
    RoomPage --> Member : join
    RoomPage --> Room : loadSnapshot
    Room --> CanvasElement : dung lai
```

> *Hình 3.3: Class diagram phân tích — Tham gia phòng.*

---

##### *Module c — Chức năng Vẽ tự do trên bảng trắng*

**Phân tích tĩnh.**

* NSD chọn công cụ + màu + độ dày → giao diện thanh công cụ → đề xuất lớp **Toolbar** `«View»`.
* NSD vẽ trên vùng bảng trắng → giao diện vùng vẽ → đề xuất lớp **Canvas** `«View»`.
* Hệ thống cần tạo/sửa/xóa phần tử và đồng bộ → cần `add()`, `modify()`, `remove()`, `synchronize()` → là hành động của **CanvasElement** `«Entity»`.
* Hệ thống cần lưu trạng thái bảng trắng định kỳ → cần `saveSnapshot(elements)` → là hành động của **Room** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Canvas** | Vùng vẽ tương tác trên bảng trắng — bắt thao tác chuột và hiển thị phần tử vẽ. |
| `«View»` | **Toolbar** | Cho phép chọn công cụ vẽ (bút, tẩy, hình chữ nhật, hình tròn, đường thẳng, mũi tên, văn bản) và tùy chỉnh màu sắc, độ dày nét. |
| `«Entity»` | **CanvasElement** | `add()` — thêm phần tử mới; `modify()` — thay đổi thuộc tính; `remove()` — xóa phần tử; `synchronize()` — đồng bộ tới các thành viên cùng phòng. |
| `«Entity»` | **Room** | `saveSnapshot(elements)` — lưu trạng thái bảng trắng định kỳ. |
...
classDiagram
    ...
    Canvas --> CanvasElement : add/modify/remove
    CanvasElement --> Room : luu snapshot
```

> *Hình 3.4: Class diagram phân tích — Vẽ tự do trên bảng trắng.*

---

##### *Module d — Chức năng Chèn ảnh vào bảng trắng*

**Phân tích tĩnh.**

* NSD dán ảnh (Ctrl+V) hoặc chọn file ảnh → sử dụng lại lớp **Canvas** `«View»` để bắt thao tác.
* Hệ thống cần kiểm tra định dạng/kích thước, tải ảnh lên kho và lấy URL → cần `validateFormat()`, `upload(file)`, `getURL()` → là hành động của **File** `«Entity»`.
* Hệ thống cần đặt ảnh tại tâm vùng nhìn và đồng bộ → cần `add()`, `synchronize()` → là hành động của **CanvasElement** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Canvas** | Bắt thao tác dán ảnh từ clipboard (Ctrl+V) hoặc chọn file ảnh. |
| `«Entity»` | **File** | `upload(file)` — tải ảnh lên kho lưu trữ; `validateFormat()` — kiểm tra định dạng và kích thước; `getURL()` — trả URL công khai. |
| `«Entity»` | **CanvasElement** | `add()` — tạo phần tử ảnh trên bảng trắng và đặt tại tâm vùng nhìn; `synchronize()` — đồng bộ tới các thành viên khác. |
...
classDiagram
    ...
    Canvas --> File : upload + validate
    Canvas --> CanvasElement : tao phan tu anh
```

> *Hình 3.5: Class diagram phân tích — Chèn ảnh vào bảng trắng.*

---

##### *Module e — Chức năng Chia sẻ màn hình*

**Phân tích tĩnh.**

* NSD A click "Chia sẻ màn hình", chọn độ phân giải/FPS/âm thanh → sử dụng lại lớp **Toolbar** `«View»`.
* Hệ thống hiển thị luồng chia sẻ dưới dạng lớp phủ trên bảng trắng → sử dụng lại lớp **Canvas** `«View»`.
* Hệ thống cần lấy màn hình, khởi/dừng phiên, đổi chất lượng, phân phối và đăng ký nhận luồng → cần `start(...)`, `stop()`, `changeResolution(res)`, `changeFrameRate(fps)`, `broadcast()`, `subscribe()` → là hành động của **ScreenShareSession** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Toolbar** | Nút "Chia sẻ màn hình" và bảng tùy chọn (độ phân giải 720p/1080p/4K, FPS 15/30/60, có/không kèm âm thanh). |
| `«View»` | **Canvas** | Hiển thị luồng video chia sẻ dưới dạng lớp phủ cho phép vẽ ghi chú đè. |
| `«Entity»` | **ScreenShareSession** | `start(source, resolution, fps, withAudio)` — yêu cầu màn hình từ thiết bị, khởi phiên; `stop()` — dừng phiên; `changeResolution(res)` / `changeFrameRate(fps)` — điều chỉnh chất lượng; `subscribe()` — đăng ký nhận luồng (phía viewer); `broadcast()` — phân phối tới các thành viên. |
...
classDiagram
    ...
    Toolbar --> ScreenShareSession : start/stop
    ScreenShareSession --> Canvas : hien thi lop phu
```

> *Hình 3.6: Class diagram phân tích — Chia sẻ màn hình.*

---

##### *Module f — Chức năng Gọi thoại (Voice Chat)*

**Phân tích tĩnh.**

* NSD bật/tắt microphone → sử dụng lại lớp **Toolbar** `«View»` (nút mic kèm chỉ báo trạng thái).
* Hệ thống cần thu/tắt micro, phân phối và đăng ký nhận âm thanh → cần `enableMicrophone()`, `disableMicrophone()`, `broadcast()`, `subscribe()` → là hành động của **VoiceSession** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Toolbar** | Nút bật/tắt microphone kèm chỉ báo trạng thái. |
| `«Entity»` | **VoiceSession** | `enableMicrophone()` — yêu cầu quyền và thu âm thanh; `disableMicrophone()` — tắt micro; `subscribe()` — đăng ký nhận âm thanh từ thành viên khác; `broadcast()` — phân phối tới các thành viên cùng phòng. |
...
classDiagram
    ...
    Toolbar --> VoiceSession : bat/tat mic
```

> *Hình 3.7: Class diagram phân tích — Gọi thoại.*

---

##### *Module g — Chức năng Gửi tin nhắn chat*

**Phân tích tĩnh.**

* NSD soạn và gửi tin nhắn, xem tin mới và badge chưa đọc → giao diện khung chat → đề xuất lớp **ChatPanel** `«View»`.
* Hệ thống cần phát tin nhắn tới phòng và xử lý tin nhận về → cần `send(text)`, `receive()` → là hành động của **ChatMessage** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **ChatPanel** | Khung tin nhắn (có thể đóng/mở), ô nhập + nút gửi; hiển thị tin nhắn mới, badge số tin chưa đọc khi khung đóng. |
| `«Entity»` | **ChatMessage** | `send(text)` — phát tin nhắn tới các thành viên trong phòng; `receive()` — xử lý tin nhắn nhận về và đưa lên giao diện. |
...
classDiagram
    ...
    ChatPanel --> ChatMessage : send / receive
```

> *Hình 3.8: Class diagram phân tích — Gửi tin nhắn chat.*

---

##### *Module h — Chức năng Rời phòng*

**Phân tích tĩnh.**

* NSD click "Rời phòng" và xác nhận → sử dụng lại lớp **RoomPage** `«View»` (nút + hộp thoại xác nhận, đồng thời cập nhật khi thành viên khác rời).
* Hệ thống cần loại NSD khỏi phòng, thông báo và cập nhật danh sách → cần `leave(room)`, `notifyOthers()`, `listAll(room)` → là hành động của **Member** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **RoomPage** | Nút "Rời phòng" + hộp thoại xác nhận; cập nhật giao diện khi một thành viên khác rời phòng. |
| `«Entity»` | **Member** | `leave(room)` — loại bỏ thành viên khỏi phòng; `notifyOthers()` — thông báo cho các thành viên còn lại; `listAll(room)` — cập nhật danh sách thành viên hiển thị. |
...
classDiagram
    ...
    RoomPage --> Member : leave
```

> *Hình 3.9: Class diagram phân tích — Rời phòng.*

---

##### *Module i — Chức năng Ghi chú đè màn hình chia sẻ bằng Desktop Overlay*

Module này đặc thù vì cần ứng dụng desktop riêng để tạo lớp phủ trong suốt trên toàn màn hình. Phần phân tích chỉ trình bày khái niệm; chi tiết cài đặt (giao thức deep link, cửa sổ Electron trong suốt, phím tắt toàn cục) thuộc về pha Thiết kế và Cài đặt.

**Phân tích tĩnh.**

* NSD A (đang chia sẻ màn hình) thấy nút "Mở Desktop Overlay" trên web → sử dụng lại lớp **RoomPage** `«View»` (web).
* Lớp phủ desktop toàn màn hình + toolbar mini + chỉ báo chế độ "Làm việc/Vẽ" → đề xuất lớp **OverlayPage** `«View»` (desktop).
* Hệ thống cần khởi động lớp phủ với quyền truy cập phòng → cần `launchOverlay()` → là hành động của **ScreenShareSession** `«Entity»`.
* Hệ thống cần tạo/xóa nét ghi chú gắn với phiên chia sẻ và đồng bộ → cần `create(points, style, shareSession)`, `erase()`, `synchronize()` → là hành động của **OverlayStroke** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **RoomPage** *(web)* | Nút "Mở Desktop Overlay" — chỉ hiển thị khi NSD đang chia sẻ màn hình. |
| `«View»` | **OverlayPage** *(desktop)* | Lớp phủ toàn màn hình trong suốt; toolbar mini; chỉ báo chế độ "Làm việc / Vẽ". |
| `«Entity»` | **ScreenShareSession** | `launchOverlay()` — yêu cầu khởi động lớp phủ desktop với quyền truy cập phòng. |
| `«Entity»` | **OverlayStroke** | `create(points, style, shareSession)` — tạo nét ghi chú gắn với phiên chia sẻ; `erase()` — xóa nét; `synchronize()` — đồng bộ tới các thành viên cùng phòng (đè lên video chia sẻ ở phía viewer). |
...
classDiagram
    ...
    RoomPage --> ScreenShareSession : launchOverlay
    OverlayPage --> OverlayStroke : create / erase
```

> *Hình 3.10: Class diagram phân tích — Desktop Overlay.*

---

##### *Module j — Chức năng Hoàn tác / Làm lại (Undo/Redo)*

**Phân tích tĩnh.**

* NSD click Hoàn tác/Làm lại → sử dụng lại lớp **Toolbar** `«View»` (hai nút + phím tắt, tự cập nhật trạng thái enable/disable).
* Hệ thống hiển thị trạng thái mới của bảng trắng → sử dụng lại lớp **Canvas** `«View»`.
* Hệ thống cần tính thao tác đảo, khôi phục và đồng bộ → cần `undo()`, `redo()`, `synchronize()` → là hành động của **CanvasElement** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Toolbar** | Hai nút Hoàn tác / Làm lại kèm phím tắt; tự cập nhật trạng thái enable/disable. |
| `«View»` | **Canvas** | Hiển thị trạng thái mới của bảng trắng sau khi đảo / khôi phục thao tác. |
| `«Entity»` | **CanvasElement** | `undo()` — đảo ngược thao tác cuối của NSD (xóa nét vừa thêm, khôi phục nét vừa xóa, hoặc trả phần tử về trạng thái trước khi sửa); `redo()` — khôi phục thao tác vừa hoàn tác; `synchronize()` — đồng bộ thao tác đảo tới các thành viên khác để giữ nhất quán. |
...
classDiagram
    ...
    Toolbar --> CanvasElement : undo / redo
    CanvasElement --> Canvas : cap nhat trang thai
```

> *Hình 3.11: Class diagram phân tích — Undo/Redo.*

---

##### *Module k — Chức năng Di chuyển và Phóng to/Thu nhỏ vùng nhìn (Pan/Zoom)*

**Phân tích tĩnh.**

* NSD lăn chuột (zoom) hoặc giữ Space + kéo (pan) → sử dụng lại lớp **Canvas** `«View»`; Canvas tự cập nhật vùng nhìn cục bộ và vẽ lại các phần tử.
* Pan/Zoom **không** thay đổi vị trí tuyệt đối của bất kỳ phần tử nào và **không** phân phối ra ngoài → module này **không phát sinh `«Entity»`** (chỉ là biến đổi vùng nhìn của View).

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Canvas** | Bắt sự kiện lăn chuột (zoom) hoặc giữ Space + kéo (pan); vẽ lại bảng trắng theo vùng nhìn mới. |
| `«Entity»` | — | Không có entity — Pan/Zoom chỉ thay đổi vùng nhìn cục bộ của NSD, không thay đổi vị trí tuyệt đối của bất kỳ `CanvasElement` nào và không phân phối ra ngoài. |
...
classDiagram
    ...
    note for Canvas "Khong co Entity: Pan/Zoom la thao tac cuc bo cua View"
```

> *Hình 3.12: Class diagram phân tích — Pan/Zoom.*

---

##### *Module l — Chức năng Hiển thị con trỏ thành viên khác (Remote Cursors)*

**Phân tích tĩnh.**

* NSD di chuyển chuột; hệ thống vẽ con trỏ remote của người khác kèm tên + màu → sử dụng lại lớp **Canvas** `«View»`.
* Hệ thống cần cập nhật vị trí, phân phối (giới hạn tần suất), nhận/hiển thị con trỏ người khác và gỡ con trỏ cũ → cần `update(x, y)`, `broadcast()`, `receiveRemote(member, position)`, `cleanupStale()` → là hành động của **Cursor** `«Entity»`.

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **Canvas** | Bắt sự kiện di chuyển chuột; vẽ con trỏ remote của các thành viên khác kèm tên hiển thị và màu đại diện. |
| `«Entity»` | **Cursor** | `update(x, y)` — cập nhật vị trí của thành viên hiện tại; `broadcast()` — phân phối vị trí tới các thành viên khác (có giới hạn tần suất để giảm tải mạng); `receiveRemote(member, position)` — nhận và hiển thị con trỏ của người khác; `cleanupStale()` — gỡ con trỏ của thành viên không cập nhật trong N giây. |
...
classDiagram
    ...
    Canvas --> Cursor : update / receiveRemote
```

> *Hình 3.13: Class diagram phân tích — Remote Cursors.*

---

##### *Module m — Chức năng Đổi tên hiển thị*

**Phân tích tĩnh.**

* NSD nhập tên hiển thị mới và xác nhận → sử dụng lại lớp **SettingsPanel** / **MembersPanel** `«View»` (ô nhập tên + nút xác nhận).
* Hệ thống cần cập nhật tên và thông báo cho người khác qua danh sách thành viên → cần `updateName(newName)`, `broadcast()` → là hành động của **Member** `«Entity»`. (Trong bản hiện tại ô tên ở chế độ chỉ-đọc nên thao tác này chưa được kích hoạt từ giao diện.)

Kết quả thu được biểu đồ lớp phân tích:

| Stereotype | Lớp | Trách nhiệm / Phương thức nghiệp vụ |
| :--- | :--- | :--- |
| `«View»` | **SettingsPanel** / **MembersPanel** | Ô nhập "Tên hiển thị" + nút xác nhận. |
| `«Entity»` | **Member** | `updateName(newName)` — cập nhật tên hiển thị; `broadcast()` — thông báo tên mới tới các thành viên còn lại qua danh sách thành viên. |
...
classDiagram
    ...
    SettingsPanel --> Member : updateName
    MembersPanel --> Member : updateName
```

> *Hình 3.14: Class diagram phân tích — Đổi tên hiển thị.*

### 3.1.3. Việc 3 — Phân tích động (Sequence Diagram phân tích)

Phân tích động mô tả thứ tự tương tác giữa các lớp đã trích ở mục 3.1.2 để hoàn thành từng kịch bản. Đúng phong cách giáo trình Nhập môn CNPM (Section 7.8.3) và thống nhất với quyết định "Phân tích chỉ có View + Entity" đã chốt ở mục 3.1.2, **mỗi sequence diagram phân tích chỉ chứa ba loại lifeline**:

- `actor` (Người sử dụng — NSD)
- `«View»` (lớp giao diện)
- `«Entity»` (lớp thực thể, gồm cả các Entity ảo ScreenShareSession / VoiceSession / OverlayStroke)

Các message giữa các lifeline được viết bằng câu nghiệp vụ kiểu "Lớp X yêu cầu lớp Y xử lý…". Việc "đồng bộ tới các thành viên khác trong phòng" được diễn đạt như một phương thức của Entity (`synchronize()`, `broadcast()`, `notifyOthers()`) — *cách hệ thống thực hiện việc đồng bộ qua Broker, kênh sự kiện cụ thể nào* sẽ được giải đáp ở pha Thiết kế (mục 3.2.5).

Để gọn báo cáo, các sequence diagram được mô tả dạng **bullet bước-bước** đi kèm danh sách lifeline. Báo cáo cuối sẽ chuyển sang biểu đồ tuần tự UML chuẩn bằng Lucidchart/Draw.io.
...
2. `LandingPage` → `Room`: yêu cầu `create()`.
3. `Room`: sinh mã phòng và mã PIN ngẫu nhiên, lưu phòng mới.
4. `Room` → `LandingPage`: trả về cặp mã phòng + PIN.
...
sequenceDiagram
    ...
    LP->>R: create()
    R->>R: sinh mã phòng + PIN, lưu phòng
...
**Hiện thực hóa (Phân tích):** `Room.verifyPasscode()/findByCode()/loadSnapshot()`, `Member.join()/listAll()`, `CanvasElement.synchronize()` — Kịch bản b. (Refine từ Hình 3.3.)
...
> *Hình 3.46: Class diagram thiết kế — Tham gia phòng (refine từ Hình 3.3).*

---

##### *Module c — Vẽ tự do trên bảng trắng*

**Hiện thực hóa (Phân tích):** `CanvasElement.add()/modify()/remove()/synchronize()`, `Room.saveSnapshot()` — Kịch bản c. (Refine từ Hình 3.4.)
...
> *Hình 3.47: Class diagram thiết kế — Vẽ tự do (refine từ Hình 3.4).*

---

##### *Module d — Chèn ảnh vào bảng trắng*

**Hiện thực hóa (Phân tích):** `File.upload()/validateFormat()/getURL()`, `CanvasElement.add()/synchronize()` — Kịch bản d. (Refine từ Hình 3.5.)
...
> *Hình 3.48: Class diagram thiết kế — Chèn ảnh (refine từ Hình 3.5).*

---

##### *Module e — Chia sẻ màn hình*

**Hiện thực hóa (Phân tích):** `ScreenShareSession.start()/stop()/changeResolution()/changeFrameRate()/broadcast()/subscribe()` — Kịch bản e. (Refine từ Hình 3.6.)
...
> *Hình 3.49: Class diagram thiết kế — Chia sẻ màn hình (refine từ Hình 3.6).*

---

##### *Module f — Gọi thoại (Voice Chat)*

**Hiện thực hóa (Phân tích):** `VoiceSession.enableMicrophone()/disableMicrophone()/broadcast()/subscribe()` — Kịch bản f. (Refine từ Hình 3.7.)
...
> *Hình 3.50: Class diagram thiết kế — Gọi thoại (refine từ Hình 3.7).*

---

##### *Module g — Gửi tin nhắn chat*

**Hiện thực hóa (Phân tích):** `ChatMessage.send()/receive()` — Kịch bản g. (Refine từ Hình 3.8.)
...
> *Hình 3.51: Class diagram thiết kế — Gửi tin nhắn chat (refine từ Hình 3.8).*

---

##### *Module h — Rời phòng*

**Hiện thực hóa (Phân tích):** `Member.leave()/notifyOthers()/listAll()` — Kịch bản h. (Refine từ Hình 3.9.)
...
> *Hình 3.52: Class diagram thiết kế — Rời phòng (refine từ Hình 3.9).*

---

##### *Module i — Ghi chú đè màn hình chia sẻ bằng Desktop Overlay*

**Hiện thực hóa (Phân tích):** `ScreenShareSession.launchOverlay()`, `OverlayStroke.create()/erase()/synchronize()` — Kịch bản i. (Refine từ Hình 3.10.)
...
> *Hình 3.53: Class diagram thiết kế — Desktop Overlay (refine từ Hình 3.10).*

---

##### *Module j — Hoàn tác / Làm lại (Undo/Redo)*

**Hiện thực hóa (Phân tích):** `CanvasElement.undo()/redo()/synchronize()` — Kịch bản j. (Refine từ Hình 3.11.)
...
> *Hình 3.54: Class diagram thiết kế — Undo/Redo (refine từ Hình 3.11).*

---

##### *Module k — Di chuyển và Phóng to/Thu nhỏ vùng nhìn (Pan/Zoom)*

**Hiện thực hóa (Phân tích):** thao tác vùng nhìn cục bộ của `Canvas` — Kịch bản k. (Refine từ Hình 3.12.)
...
> *Hình 3.55: Class diagram thiết kế — Pan/Zoom (refine từ Hình 3.12).*

---

##### *Module l — Hiển thị con trỏ thành viên khác (Remote Cursors)*

**Hiện thực hóa (Phân tích):** `Cursor.update()/broadcast()/receiveRemote()/cleanupStale()` — Kịch bản l. (Refine từ Hình 3.13.)
...
> *Hình 3.56: Class diagram thiết kế — Remote Cursors (refine từ Hình 3.13).*

---

##### *Module m — Đổi tên hiển thị*

**Hiện thực hóa (Phân tích):** `Member.updateName()/broadcast()` — Kịch bản m. (Refine từ Hình 3.14.)
...
> *Hình 3.57: Class diagram thiết kế — Đổi tên hiển thị (refine từ Hình 3.14).*
...
stateDiagram-v2
    ...
    Active --> Active : làm tươi hoạt động phòng (canvas_op, cursor_move, chat message)
...
**State Diagram 3 — `OverlayMode` (Desktop):**

```mermaid
stateDiagram-v2
    [*] --> Drawing : mở overlay cho phiên chia sẻ (khóa vùng nhìn theo vùng chia sẻ)
    Drawing --> Working : Ctrl+Shift+D (xuyên chuột)
...
> *Hình 3.29–3.31: Biểu đồ trạng thái cho Room, ScreenShareSession, OverlayMode.*
